...
(   TEID BIGINT,
    ...
    CONSTRAINT TEMPLATEKEY02 FOREIGN KEY (EVID) REFERENCES event(evid),
    CONSTRAINT TEMPLATEKEY03 FOREIGN KEY (ORID) REFERENCES origin(orid),
    CONSTRAINT TEMPLATEKEY04 FOREIGN KEY (PROCCOMMID) REFERENCE remark(commid)
);

COMMENT ON TABLE TEMPLATE IS "Matched filter template table";
COMMENT ON COLUMN TEMPLATE.TEID IS "Unique numerical identifier for each template";
COMMENT ON COLUMN TEMPlATE.EVID IS "Event ID that this template is based on";
COMMENT ON COLUMN TEMPLATE.ORID IS "Origin ID that this template is based on";
COMMENT ON COLUMN TEMPLATE.NAME IS "EQcorrscan Template object name attribute";
COMMENT ON COLUMN TEMPLATE.PROCCOMID IS "Comment ID corresponding to template construction parameters"
